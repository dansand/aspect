        ...
        if  (this->simulator_is_past_initialization() && this->get_timestep_number() > 0 && in.strain_rate.size())
          {
            ...
            //this section determines the combination of plastic strain and healing
            if (weakening_mechanism != none || weakening_mechanism != viscous_weakening_with_viscous_strain_only || weakening_mechanism != finite_strain_tensor)
              {
                // get the current value of the plastic (or total) strain
