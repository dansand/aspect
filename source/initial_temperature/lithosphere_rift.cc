...
namespace aspect
{
  namespace InitialTemperature
  {
    ...
    initial_temperature (const Point<dim> &position) const
    {
      ...
      for (typename std::list<std::unique_ptr<InitialComposition::Interface<dim> > >::const_iterator it = this->get_initial_composition_manager().get_act    ive_initial_composition_conditions().begin();
